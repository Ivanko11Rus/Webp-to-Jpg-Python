...
output_path = r""              # папка для JPG (пусто = создаст подпапку overkill_converted в папке, над которой ведётся работа)
...
def validate_settings():
    ...
    # Проверка overkill_mode (должно быть bool)
    if not isinstance(overkill_mode, bool):
        errors.append(f"overkill_mode должно быть True или False, получено: {overkill_mode} (тип {type(overkill_mode).__name__})")
    ...
    # Проверка save_in_place
    if not isinstance(save_in_place, bool):
        errors.append(f"save_in_place должно быть True или False, получено: {save_in_place} (тип {type(save_in_place).__name__})")
    ...
    # Проверка on_recycle_error
    valid_recycle_error = ["ask", "permanent", "skip", "stop"]
    if on_recycle_error not in valid_recycle_error:
        errors.append(f"on_recycle_error должно быть одним из {valid_recycle_error}, получено: {on_recycle_error}")
...
if overkill_mode:
    ...
    # Показываем предупреждения для delete_on_skip и delete_on_rename только если delete_original = True
    if delete_original:
        if delete_on_skip:
        ...
        if delete_on_rename:
            print(f"{Fore.RED}❗❗❗ ВНИМАНИЕ: ВКЛЮЧЁН РЕЖИМ УДАЛЕНИЯ ПРИ ПЕРЕИМЕНОВАНИИ! WebP будут удалены, если при конфликте выбрано переименование.{Style.RESET_ALL}")
    print()
...
for idx, src_path in enumerate(webp_files, start=1):
    ...
    dst_path = None
    action_performed = False  # флаг, что файл был обработан (пропущен или переименован)
    ...
    if existing_jpgs:
        ...
        if on_name_conflict == "skip":
            ...
            # Неизвестное значение – действуем как rename (или пропуск? но лучше как rename)
            show_conflict_info(base_name, existing_jpgs, original_size, dest_dir)
            next_num = get_next_number(existing_jpgs)
            new_filename = f"{base_name} ({next_num}).jpg"
            dst_path = os.path.join(dest_dir, new_filename)
